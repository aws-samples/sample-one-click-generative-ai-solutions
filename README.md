...
[AI ペルソナシステム](https://github.com/aws-samples/sample-ai-persona) は、Amazon Bedrock を活用し、AIペルソナの構築と、ペルソナ同士の議論・インタビュー・アンケート調査を通じて商品企画やマーケティング戦略立案のためのインサイトを生成するサンプル実装です。

 [![](https://s3.amazonaws.com/cloudformation-examples/cloudformation-launch-stack.png)](https://us-east-1.console.aws.amazon.com/cloudformation/home#/stacks/create/review?stackName=AIPersonaDeploymentStack&templateURL=https://aws-ml-jp.s3.ap-northeast-1.amazonaws.com/asset-deployments/AIPersonaDeploymentStack.yaml)
...
* **インタビュー**: AIペルソナとの議論・対話を通じたインサイト発見（簡易議論・しっかり議論・リアルタイムインタビューの3モード）
* **アンケート調査**: 数百〜数千のAIペルソナへの大規模アンケート実施
* **ペルソナ生成**: 多様なデータ＋自然言語の指示でAIペルソナを自動生成

## Spec-Driven Presentation Maker (SDPM)

[Spec-Driven Presentation Maker](https://github.com/aws-samples/sample-spec-driven-presentation-maker) は、プレゼンテーション資料を生成 AI で作成できるオープンソースのアプリケーションです。仕様駆動開発のアプローチを用いて、「何を伝えるか」を先に設計し、「どう見せるか」を AI が構築します。Kiro CLI スキルからフルスタック Web アプリまで、4 層アーキテクチャで必要なレイヤーだけ選んで使えます。通常は layer4（フルスタック）をデプロイしてください。Kiro などのコーディングエージェントで利用したい場合は、AWS デプロイ不要の [Layer 1: Kiro CLI スキル](https://github.com/aws-samples/sample-spec-driven-presentation-maker/blob/main/docs/ja/getting-started.md#layer-1-kiro-cli-%E3%82%B9%E3%82%AD%E3%83%AB) を参照してください。

 [![](https://s3.amazonaws.com/cloudformation-examples/cloudformation-launch-stack.png)](https://ap-northeast-1.console.aws.amazon.com/cloudformation/home#/stacks/create/review?stackName=SdpmDeploymentStack&templateURL=https://aws-ml-jp.s3.ap-northeast-1.amazonaws.com/asset-deployments/SdpmDeploymentStack.yaml)

### Parameters

* NotificationEmailAddress
   * デプロイの開始・終了を通知するメールアドレスです。
* DeploymentLayer (default: layer4)
   * デプロイするレイヤーです。`layer3` は MCP サーバーのみ、`layer4` は Agent + Web UI を含むフルスタックです。
* ModelId (default: global.anthropic.claude-sonnet-4-6)
   * Agent が使用する Bedrock モデル ID です（例: global.anthropic.claude-sonnet-4-6, global.anthropic.claude-opus-4-6-v1）
* EnableInvocationLogging (default: false)
   * Bedrock Model Invocation Logging の有効/無効を切り替えます
* AllowedIpV4AddressRanges
   * アクセスを許可する IPv4 アドレス範囲（CIDR）です。指定することでアクセス元 IP を制限できます。
* AllowedIpV6AddressRanges
   * アクセスを許可する IPv6 アドレス範囲（CIDR）です。指定することでアクセス元 IP を制限できます。

### 主な機能

* **仕様駆動設計**: ソース資料から論理構造を設計書として定義
* **AI 自動構築**: テンプレートに準拠して AI がスライドを自動構築
* **4 層アーキテクチャ**: Kiro CLI スキルからフルスタック Web アプリまで
* **MCP 対応**: Claude Desktop, VS Code, Kiro 等の MCP クライアントで利用可能
