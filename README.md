...
[Spec-Driven Presentation Maker](https://github.com/aws-samples/sample-spec-driven-presentation-maker) は、仕様駆動開発のアプローチでプレゼンテーション資料を作成するオープンソースツールキットです。「何を伝えるか」を先に設計し、「どう見せるか」を AI が構築します。Kiro CLI スキルからフルスタック Web アプリまで、4 層アーキテクチャで必要なレイヤーだけ選んで使えます。通常は layer4（フルスタック）をデプロイしてください。Kiro などのコーディングエージェントで利用したい場合は、AWS デプロイ不要の [Layer 1: Kiro CLI スキル](https://github.com/aws-samples/sample-spec-driven-presentation-maker/blob/main/docs/ja/getting-started.md#layer-1-kiro-cli-%E3%82%B9%E3%82%AD%E3%83%AB) を参照してください。
...
* SearchSlides (default: false)
   * セマンティックスライド検索の有効/無効を切り替えます（Bedrock Knowledge Base を使用）
